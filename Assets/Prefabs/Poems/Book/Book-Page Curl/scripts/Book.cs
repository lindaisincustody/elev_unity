...
public class Book : MonoBehaviour {
    ...
    public void UpdateBookRTLToPoint(Vector3 followLocation)
    {
        ...
        int childCount = NextPageClip.transform.childCount;
        int secondToLastIndex = Mathf.Max(childCount - 2, 0);
        RightNext.transform.SetSiblingIndex(secondToLastIndex);
        c = Calc_C_Position(followLocation);
        Vector3 t1;
    ...
    public void ReleasePage(bool showPoem)
    {
        if (pageDragging)
        {
            pageDragging = false;
    ...
    public void TweenForward(bool showPoem)
    {
        if(mode== FlipMode.RightToLeft)
